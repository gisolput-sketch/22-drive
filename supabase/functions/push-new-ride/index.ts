...
const VAPID_PUBLIC_KEY =
  ...
  "BPv-5ZgA0eonKTtctG8P2Zj1sQKZaqKDvgmzPb3oC9ilScCllIpERnY_SPsmieYZgWEV2nhzqqAo8Og_YEnva8M";

const VAPID_PRIVATE_KEY = Deno.env.get("VAPID_PRIVATE_KEY") ?? "";

const VAPID_SUBJECT =
  Deno.env.get("VAPID_SUBJECT") ??
  "mailto:22drive@example.com";
...
serve(async (req) => {
  ...
  try {
    if (!SUPABASE_URL) {
      throw new Error(
        "SUPABASE_URL não está disponível."
      );
    }

    if (!SERVICE_ROLE_KEY) {
      throw new Error(
        "A chave secreta do Supabase não está disponível no ambiente da Edge Function."
      );
    }

    if (!VAPID_PUBLIC_KEY || !VAPID_PRIVATE_KEY) {
      throw new Error(
        "As chaves VAPID não estão configuradas."
      );
    }
