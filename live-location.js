(() => {
  ...
  function haversine(a,b){if(!a||!b)return Infinity;const R=6371000,rad=Math.PI/180,dLat=(b[0]-a[0])*rad,dLon=(b[1]-a[1])*rad,x=Math.sin(dLat/2)**2+Math.cos(a[0]*rad)*Math.cos(b[0]*rad)*Math.sin(dLon/2)**2;return 2*R*Math.asin(Math.sqrt(x));}
  ...
  function openWhatsAppTracking(r){
    const phone=String(r?.passengerPhone||r?.passenger_phone||r?.telefone||r?.whatsapp||'').replace(/\D/g,'');const tokenUrl=getTrackingUrl(r);if(!phone||!tokenUrl)return false;const n=phone.startsWith('55')?phone:'55'+phone,NL='\n';
  ...
  async function stopSharing(clear=true){
    ...
    shareWatchId=null;const id=sharingRideId;sharingRideId=null;lastSentAt=0;lastSentPosition=null;if(clear&&id)await updateReservation(id,{driver_lat:null,driver_lng:null,driver_location_at:null});
  }
  async function publishPosition(id,pos,force=false){
    const lat=Number(pos.coords.latitude),lng=Number(pos.coords.longitude);if(!Number.isFinite(lat)||!Number.isFinite(lng))return;const point=[lat,lng],now=Date.now();
    if(!force&&now-lastSentAt<UPDATE_MIN_MS&&haversine(point,lastSentPosition)<UPDATE_MIN_METERS)return;lastSentAt=now;lastSentPosition=point;
  ...
  function startSharing(id,sendWhatsApp=true){
    const ride=getRide(id);if(!ride||!navigator.geolocation)return;if(sharingRideId===String(id)&&shareWatchId!==null)return;stopSharing(false);sharingRideId=String(id);
    navigator.geolocation.getCurrentPosition(pos=>publishPosition(id,pos,true),()=>{},WATCH_OPTIONS);shareWatchId=navigator.geolocation.watchPosition(pos=>publishPosition(id,pos,false),err=>console.warn('22 DRIVE GPS compartilhado:',err),WATCH_OPTIONS);
  ...
  function ensureDriverWrappers(){
    ...
    {const originalFinish=window.finishRide;if(typeof originalFinish==='function'&&!originalFinish.__liveWrapped){const fixedFinish=async function(...args){const result=await originalFinish.apply(this,args);const id=args[0];setTimeout(()=>{try{const ride=getRide(id);if(ride&&String(ride.status).toLowerCase()==='finished')stopSharing(true);}catch(_){}},900);return result;};fixedFinish.__liveWrapped=true;window.finishRide=fixedFinish;}}
    wrap('autoFinishAtDestination',(args)=>{const ride=args[0];const id=ride&&ride.id;if(id!=null)stopSharing(true);});wrap('logout',()=>stopSharing(true));
  }
  function buildPassengerTrackingCard(){
    if(!isPassenger||$('liveTrackingCard'))return;const statusCard=$('tripStatusCard');if(!statusCard)return;const card=document.createElement('section');card.className='card';card.id='liveTrackingCard';card.style.display='none';
    card.innerHTML='<h2>🚗 Acompanhar motorista</h2><div id="liveTrackingStatus" class="notice">Aguardando o motorista iniciar a rota.</div><button id="liveTrackingCenter" class="btn primary" type="button" style="margin-top:9px;display:none">📍 Ver motorista no mapa</button>';statusCard.insertAdjacentElement('afterend',card);
    $('liveTrackingCenter').addEventListener('click',()=>{try{if(typeof passengerDriverMarker!=='undefined'&&passengerDriverMarker&&typeof driveMap!=='undefined'&&driveMap){driveMap.setView(passengerDriverMarker.getLatLng(),16,{animate:true});return;}}catch(_){}if(passengerTrackingMarker&&passengerTrackingMap)passengerTrackingMap.setView(passengerTrackingMarker.getLatLng(),16,{animate:true});});
  }
  function updatePassengerTracking(r){
    if(!isPassenger||!r)return;buildPassengerTrackingCard();const card=$('liveTrackingCard'),status=$('liveTrackingStatus'),center=$('liveTrackingCenter');const lat=Number(r.driver_lat),lng=Number(r.driver_lng),sharing=Number.isFinite(lat)&&Number.isFinite(lng)&&String(r.status||'').toLowerCase()!=='finished';
    ...
    status.innerHTML='🟢 <b>Motorista em rota</b><br>📍 A localização está sendo atualizada em tempo real.';if(center)center.style.display='block';if(typeof window.updatePassengerDriverMap==='function'){try{window.updatePassengerDriverMap(r);}catch(_) {}}
  }
  function watchPassengerReservation(){
    if(!isPassenger||!window.supabase)return;const id=localStorage.getItem('22drive_last_reservation_id');if(!id)return;const sb=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY);
  ...
  function initTrackingPage(){
    if(!isTrackingPage)return;const token=new URLSearchParams(location.search).get('token');if(!token)return showTrackingError('Link de acompanhamento inválido.');if(!window.supabase||!window.L)return setTimeout(initTrackingPage,300);
    const sb=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY),map=L.map($('trackingMap'),{zoomControl:true}).setView([-22.73,-42.63],13);L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap contributors'}).addTo(map);passengerTrackingMap=map;
